...
Find <- function(pattern, original, comparator = function(a,b){return(a==b)}){
  ...
  #
  # Args:
  #   pattern: subvector to look for
  #   original: vector to search in
  #   comparator: function tha compares two elements
  #
  # Returns:
  #   Vector with start indexes of entries of subvector, if pattern was founded. Otherwise, zero length vector.
  ...
  # Finds all entries of the first element of subvector.
...
PerformRefactor <- function(contents, from, to, useWordBoundaries=FALSE) {
  # Find patternt in content and replace it.
  #
  # Args:
  #   contents: content to search in
  #   from: regular expression, that will be replaced
  #   to: string that will be replacement
  #
  # Returns:
...
Echo <- function(content, context){
  ...
  ref.result <- PerformRefactor(content, from="knitr::opts_chunk$set(echo\\s*=\\s*FALSE)", to="knitr::opts_chunk$set(echo = TRUE)")
  if (ref.result$changes == 0){
    ref.result <- PerformRefactor(content, from="echo\\s*=\\s*FALSE", to="echo = TRUE")
  }
  ref.result$refactored  # return as character vector
...
CopyAndCompare <- function(echo.true.report, fair.id, name, current.path){
  result <- Ignore()
  if ( ! result){
    message("WRITING TO GITIGNORE FAILED")
  }
